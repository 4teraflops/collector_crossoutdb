...
def collect_diff_data_to_bd():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "DELETE from difference_sell_price")  # предварительно затираем то, что было в таблице difference_sell_price
    one_day_ago_time = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d %H:%M:%S")
    one_day_ago_time_allowence = (datetime.now() - timedelta(days=1) + timedelta(minutes=30)).strftime(
        "%Y-%m-%d %H:%M:%S")
    ...
    month_ago_time_allowence = (datetime.now() - timedelta(days=30) + timedelta(minutes=30)).strftime(
        "%Y-%m-%d %H:%M:%S")
    ...
    for api_id in api_ids:
        ...
        actual_sell_price = \
        cursor.execute(f'SELECT sell_price FROM actual_data WHERE api_id = "{api_id}"').fetchall()[0][0]

        sell_price_one_day_ago = get_sell_price_from_historycal_data(api_id, one_day_ago_time,
                                                                     one_day_ago_time_allowence)
        ...
        cursor.execute(
            f"INSERT INTO difference_sell_price VALUES (Null, '{api_id}', '{name2}', '{rarity}', '{actual_sell_price}', '{one_day_sign}', '{abs(diff_sell_price_one_day)}', '{three_days_sign}', '{abs(diff_sell_price_three_days)}', '{week_sign}', '{abs(diff_sell_price_week)}', '{month_sign}', '{abs(diff_sell_price_month)}')")
        conn.commit()
